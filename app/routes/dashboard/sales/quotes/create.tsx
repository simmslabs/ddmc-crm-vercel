...
import { useCustomers, useInventories, useMediaSizes, useMediaTypes, useSalesInvoices, useSalesQuotes } from "~/services/hooks";
...
const SalesIvoiceCreatePage = () => {
  ...
  const [invoices, setInvoices] = useSalesQuotes();
  ...
  useEffect(() => {
    setOrderNumber(`SALE-${new Date().getTime()}`)
  }, []);
      ...
      <Card loading={loading} actions={[
        ...
        <Space align="center" size="large">
          ...
          {invoiceItems.length > 0 &&
            <Button onClick={(e) => saveInvoice()} color="green" icon={<SaveFilled />}>{invoice ? 'Update' : 'Save'} Quote</Button>
          }
        </Space>
      )}>
